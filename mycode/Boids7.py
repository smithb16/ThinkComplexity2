...
class Boid(cone):
    ...
    def line_of_sight(self, boids, radius=10, angle=np.pi/2):
        """Find the center of mass of all boids blocking the
        line of sight and return a vector pointing away from it."""
        neighbors = self.get_neighbors(boids, radius, angle)
        blocking = []
        for boid in neighbors:

            # find vector pointing from self to boid
            w = boid.pos - self.pos
            # find projection of w onto velocity
            t = dot(w, self.vel)
            # find the component of w that is orthogonal to velocity
            orth = w - t * self.vel
            # if orthogonal is less than boid cone diameter
            if orth.mag < 2*self.cone_radius:
                blocking.append(orth)

        return -self.vector_toward_center(blocking)
    ...
    def love(self, carrot):
        """Returns a vector pointing towards the carrot."""
    ...
    def set_goal(self, boids, carrot):
        ...
        self.goal = (   w_center * self.center(boids) +
                        ...
                        w_love * self.line_of_sight(boids))
        self.goal.mag = 1
